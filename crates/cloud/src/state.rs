...
mod tests {
    ...
    // Every test runs on the same process-wide singleton, so we must
    // clear state between cases and avoid running them in parallel.
    // The top-level test harness already uses `--test-threads=1`
    // (per `scripts/run_tests_with_openai.sh`) but we also guard each
    // test with an explicit clear at the top for safety.
    ...
    #[tokio::test]
    async fn is_connected_tracks_client_presence() {
        clear_client().await;
    ...
    #[tokio::test]
    async fn set_client_replaces_previous() {
        clear_client().await;
        let first = dummy_client();
        set_client(first.clone()).await;

        let second = CloudClient::new("https://other.example.com", "other-key").expect("valid key");
    ...
    #[tokio::test]
    async fn python_compat_aliases_match_canonical_helpers() {
        clear_client().await;
    ...
    #[tokio::test]
    async fn concurrent_reads_are_non_exclusive() {
