...
impl PayloadMetaInfo {
    pub fn new(task_counter : u32) -> Self {
        Self { id: Uuid::new_v4(), task_counter  }
    }
    pub fn task_done (&mut self) { self.task_counter += 1; }
}
...
impl<T> PayloadBase<T> {
    pub fn new(task_counter: u32, data: T) -> Self {
        Self { metainfo: PayloadMetaInfo::new(task_counter), data }
    }
}
...
mod tests {
    ...
    struct DataPoint { path: String, chunks: Vec<String> }

    #[test]
    fn constructs_and_increments_counter() {
        let dp = DataPoint { path: "a".into(), chunks: vec!["x".into()] };
    ...
    #[test]
    fn serde_roundtrip() {
        let dp = DataPoint { path: "a".into(), chunks: vec!["x".into(), "y".into()] };
    ...
    fn test() {
        let mut items: Vec<Box<dyn PayloadBehavior>> = vec![
            Box::new(CogneePayload::new(0, "Intro", vec!["hello world".into(), "lorem ipsum".into()])),
            Box::new(LowLevelPayload::new(2, 1920, 1080, vec!["tile_a".into(), "tile_b".into()])),
        ];
