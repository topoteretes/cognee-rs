...
- **Feature-gated tests:** e.g. `#![cfg(feature = "fs")]` for filesystem-specific session tests. A file-level `#![cfg(feature = ...)]` **also needs a matching `[[test]] required-features` entry** in that crate's `Cargo.toml` — without one, a lane that does not enable the feature still builds the target and reports a green `running 0 tests`, indistinguishable from a lane that ran them. Every such target in the repo is wired this way (SDK-666), so **a new feature-gated test file needs its `[[test]]` entry added at the same time** — nothing detects a missing one. The single exception **among feature-gated targets** is `crates/telemetry/tests/noop_fallback.rs`: it is gated on `cfg(not(feature = "telemetry"))`, and `required-features` is a conjunction of features that must be ON, so it cannot be declared and still reports a green zero wherever `telemetry` is enabled. It is covered instead by a dedicated `--no-default-features` lane in both `scripts/check_all.sh` and `ci.yml` — that lane is the only thing that runs it, so do not drop it (its manifest says so at the `[[test]]` group). Three more targets are gated on something other than a feature, so `required-features` — which takes feature names only — cannot express them either, and they stay green zeros on every native lane: `crates/chunking/tests/wasm.rs` and `wasm_browser.rs` (`cfg(target_arch = "wasm32")`), of which only `wasm.rs` is actually executed anywhere in CI (the wasm32 lane runs it under Node; `wasm_browser.rs` is compile-checked there and needs a browser + WebDriver to run, so it is a manual local step); and `crates/database/tests/pipeline_run_attribution.rs` (`cfg(any())`), quarantined pending its T3 re-home, which therefore runs **nowhere** — nothing will notice if it is never restored.
...
**Architecture:** multi-stage Dockerfile packaging a Python cognee venv and the pytest harness. The Rust `cognee-cli` / `cognee-telemetry-emit` release binaries are **not** built in the image — since PR #133 (`4db8bbf3`) they are built on the runner (or by hand locally) and copied in from `cognee-rust/dist/`, so the Dockerfile has no Rust stage and never invokes `cargo`. Tests run in pytest on a tmpfs workspace.
...
`ci.yml` runs on push/PR to main: lint (fmt + check + clippy), tests (with `OPENAI_KEY` secret via `scripts/run_tests_with_openai.sh`), `cargo doc --no-deps`, and C/Python/TS/Java binding checks. Three dedicated Postgres lanes — `pgvector-postgres`, `pg-single-db-postgres` and `pggraph-postgres` — run the service-container suites listed above; each ends with `scripts/ci/assert_pg_suite_ran.sh`, which fails the lane if any case skipped or if the target compiled to fewer tests than its floor (the `pgvector-spans` lane it replaced reported green for its whole life while running zero tests). The keyless ones are mirrored in `community.yml` for fork PRs. Every gating job must appear in the `notify` aggregator's `needs:` list **and** its `needs.*.result` expression — a job missing from either runs, but its result is not rolled up into the aggregate the board reports. (Note that `main` currently has **no** required status checks at all: protection is one approving review, with no rulesets. So no workflow mechanically blocks a merge today; the `notify` roll-up is what a human reads.) `http-parity.yml` runs the cross-SDK Rust↔Python parity suite on push/PR to main (it is *not* `workflow_dispatch`-only). Despite the name, it is now the **CLI/DB lane only** — COGX import + roundtrip contracts, telemetry parity, provenance parity and logging parity, driven through the two CLIs and the databases they produce. The HTTP parity phases left this repo with `cognee-http-server` and now run on the closed `cognee-cloud-rs` board, along with the websocket, sync, permissions and visualize specialty tests; there is no longer a phase-1/phase-3 split here and no `run_phase3` input. The telemetry-parity and COGX-import steps need no secrets; LLM-gated steps use `OPENAI_KEY` and skip cleanly when absent (fork-safe). `ts-prebuild.yml` builds Neon prebuilt binaries for multiple platforms (publishes the `cognee-ts` npm package).
