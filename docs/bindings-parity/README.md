...
|----|------|---------|-----------|------|--------|------|
| CR-1 | Eliminate reachable `unwrap()` on FFI paths; enforce panic safety (`catch_unwind` + `panic = "abort"`) | C API | Correctness | P0 | Not started | [01-capi-panic-safety.md](01-capi-panic-safety.md) |
| CR-2 | Fix empty-task pipeline in `execute_async`/`execute_in_background` | C API | Correctness | P0 | Not started | [02-capi-pipeline-async-tasks.md](02-capi-pipeline-async-tasks.md) |
