...
| # | Decision | Resolution |
|---|---|---|
| 1 | Java source/target floor | **11** (bytecode 55). `maven.compiler.release=11`. |
...
| T06 | Core pipeline ops: `add`, `cognify`, `addAndCognify` + typed results | not-started | T05 | `add(...).join()` returns a typed `AddResult` against sqlite temp dir (no LLM); JUnit green. |
| T07 | Retrieval ops: `search`, `recall` + `SearchType` enum + result types | not-started | T05 | `SearchType` maps to exact wire strings; `search`/`recall` compile and deserialize canned JSON; JUnit green. |
| T08 | Memory ops: `remember`, `rememberEntry`, `memify`, `improve` | not-started | T05 | All four wired with builders matching the camelCase opts; JUnit green (deserialization + arg-marshalling unit tests). |
| T09 | Data + dataset ops: `forget`, `update`, `pruneData`, `pruneSystem`, `datasets().*` | not-started | T05 | `datasets().list()`/`listData()`/`has()`/`status()`/`empty()`/`deleteData()`/`deleteAll()` + `forget`/`update`/prune wired; `add` + `datasets().list()` deterministic test passes; JUnit green. |
