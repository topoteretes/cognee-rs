...
| P1 | [p1-auth.md](p1-auth.md) | JWT + cookie + `X-Api-Key` auth, login / logout / me, `AuthenticatedUser` extractor, register / reset / verify (email-stubbed), `users` CRUD, `users/get-user-id`, api-keys CRUD. SeaORM migration for `users`, `user_api_key`. | 2 days | **In Progress** |
