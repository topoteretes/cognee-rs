...
None — this is the first phase. P0 must land before any other implementation phase begins. The `cognee_core::PipelineRunRegistry` trait that `AppState::pipelines` will eventually hold does not exist yet; this phase lands the slot as `Option<Arc<dyn PipelineRunRegistry>>` defaulting to `None` and lets P3-pre / P3 fill it in.
...
- **Action**: Define `#[derive(Clone)] pub struct AppState` with the fields listed in [architecture.md §6](../architecture.md#6-application-state--dependency-injection): `lib`, `auth`, `pipelines`, `spans`, `sync`, `config`. For P0, downscope every field that depends on un-landed phases:
  ...
  - `pub pipelines: Option<Arc<dyn cognee_core::PipelineRunRegistry>>` — `None` for now. Annotate with `// TODO(P3): replace with concrete registry from cognee-core` so the slot is unmissable.
