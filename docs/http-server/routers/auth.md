...
1. ~~**`/me` shape divergence vs auth.md**.~~ **Resolved.** [../auth.md §8.3](../auth.md#83-apiv1authme--get) now correctly states the `{"email": "<str>"}` narrow shape matching Python's `get_auth_router.py`. This doc's guidance (emit only `email`) is canonical. No further action needed.
