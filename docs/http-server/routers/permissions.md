...
5. **Default single-user tenant lookup** — when `users.tenant_id IS NULL`, the bootstrap default-tenant from [../tenants.md §6](../tenants.md#6-bootstrap-default-user--default-tenant) is the implicit fallback. Confirm this matches Python's behavior at the implementation phase.
