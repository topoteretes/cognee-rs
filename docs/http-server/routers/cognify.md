...
- **Request body**: `application/json`, DTO `CognifyPayloadDTO`. Field-by-field mapping:
  ...
  | `chunks_per_batch` | `Optional[int]` | `chunks_per_batch` | `Option<u32>` | `None` | When set, overrides `CognifyConfig::chunks_per_batch`. When `None`, the config's own default applies — `cognee_cognify::config::DEFAULT_CHUNKS_PER_BATCH`, which matches Python's 2000 batch literal. Note the router does not thread `llm_max_parallel_requests` through, so `DEFAULT_MAX_PARALLEL_EXTRACTIONS` is the effective in-flight ceiling on this path regardless of that setting. |
