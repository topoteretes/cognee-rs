...
- **Side effects**:
  ...
  - **Concurrency (SDK-636)**: the whole handler — name lookup, insert, grant, and the compensating rollback — runs under a per-identity lock keyed on `uuid5(name, owner, tenant)` (`AppState::dataset_locks`, `cognee_ingestion::DatasetLocks`). `POST /api/v1/add` takes the same lock inside its own resolve-or-create step, so the two cannot interleave. **In-process only**: two replicas over one database still race, which needs an advisory lock in the store or an ACL sharing the metadata transaction.
