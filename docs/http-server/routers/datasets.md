...
- **Permission gate**: `read` on each candidate dataset (the SDK call already filters; the handler does not re-check). [../tenants.md §5](../tenants.md#5-permission-resolution).
  ...
  **Which source answers is decided up front — by whether the ACL is wired *and* enforced — never by what the ACL returned (SDK-637).**

  - **`AclDb` wired and `REQUIRE_AUTHORIZATION` not disabled** (the cloud build): `authorized_dataset_ids_with_roles(user.id, "read")` is the whole answer, the empty list included. This matches Python — `get_datasets_router.py` returns whatever `get_all_user_permission_datasets(user, "read")` gives — and matches `SearchOrchestrator::readable_dataset_ids`, which has consulted the ACL and nothing else since [PR 214](https://github.com/topoteretes/cognee-rs/pull/214).
  - **Otherwise** — no `AclDb` (OSS single-user), or `REQUIRE_AUTHORIZATION=false|0|no`: `IngestDb::list_datasets_by_owner(user.id)`, filtered on the caller's tenant. `list_datasets_by_owner` spans every tenant the owner appears in and the bindings let one handle write under several, so without the predicate a caller scoped to tenant A sees tenant B's rows listed and is then refused them by search. A `None` tenant applies no filter — the single-tenant default every OSS row is written under, so the OSS path is unchanged.

  `REQUIRE_AUTHORIZATION` belongs in that condition because it is this server's documented "do not enforce the ACL" switch (Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` parity) and `check_permission_via_handles` honours it on **every other** dataset route — status, data, graph, raw file, write, and both deletes. A listing that ignored it would return an empty list for datasets all of those routes still serve.

  ⚠️ **Two known divergences from Python remain, both deliberate.**

  1. **No tenant predicate on the ACL path.** Python applies `dataset.tenant_id == user.tenant_id` to the *deduplicated union* of direct, tenant and role grants (`get_all_user_permission_datasets.py`), i.e. on the ACL path too. Neither this handler nor `SearchOrchestrator::readable_dataset_ids` does, so a caller in tenant A holding a direct grant on a tenant-B row is listed here and dropped by Python. Adding it to this handler alone would make the listing *stricter* than search and re-open the same disagreement in the opposite direction; adding it to both means changing the authorization gate PR 214 settled. Tracked as follow-up, not bundled here.
  2. **`is_none_or`, not `==`, on the ownership path.** The predicate is `user.tenant_id.is_none_or(|t| ds.tenant_id == Some(t))`, identical to search's — which is what makes the two agree. For a tenanted caller it coincides with Python's `==`; for an untenanted one it does not, since Python drops a tenanted row for a `tenant_id = None` caller while this admits it. That is an OSS-compat choice: a `None` tenant means "the caller named no tenant", and `==` would hide every tenanted row from the default user.

  ⚠️ **This was a behaviour change for deployments relying on the old fallback.** The guard used to be `if datasets.is_empty()`, so the ownership listing fired *through* a live ACL: a caller whose `read` grants were revoked or never written got the full ownership listing here and a `403` from `POST /v1/search` for the same datasets. Removing it means a cloud tenant whose ACL rows were never backfilled now sees an empty list rather than reading through to ownership — correct, and what search has always answered, but visible. The handler names that case rather than leaving it silent: when the response is empty **and** the caller owns rows, it logs a `warn!` carrying `user_id`, `owned_datasets` and `grants`. `grants` separates the two states that reach the same empty body and need opposite fixes — **zero grants** means the ACL needs a backfill, **grants that resolved to no row** means the ACL carries entries for deleted datasets and needs sweeping. A caller who genuinely owns no datasets logs nothing.

  The same asymmetry is worth knowing about on the write side: the ingest-side create path can still leave a dataset row whose owner grant failed (issue [212](https://github.com/topoteretes/cognee-rs/issues/212) item 18), and such a row is now invisible to this listing rather than being papered over by ownership. See [search.md §Permission gate](search.md).
...
- **Side effects**:
  ...
  - **Concurrency (SDK-636)**: the whole handler — name lookup, insert, grant, and the compensating rollback — runs under a per-identity lock keyed on `uuid5(name, owner, tenant)` (`AppState::dataset_locks`, `cognee_ingestion::DatasetLocks`). Every other in-process writer of that identity takes the same lock: the ingest paths inside `persist_data_with_acl_and_locks` (`POST /api/v1/add`, `POST /api/v1/remember`, the `PATCH /api/v1/update` re-add, and both `improve` session stages), and the two delete routes (`DELETE /api/v1/datasets/{dataset_id}` and `DELETE /api/v1/datasets`, which locks each id in turn). The delete routes matter because a half-created row is already visible to `list_datasets_by_owner`, so an unlocked sweep would delete it mid-window and leave the create answering 200 for a dataset that is gone. The lock covers resolution **by id as well as by name** — the deterministic id is derivable from the name, so a half-created row is reachable by id mid-window without any listing at all. (SDK-637 narrowed the other half of that argument: `GET /v1/datasets` now reads through to ownership only when no `AclDb` is wired, so under the cloud build a grantless half-created row is no longer listed.) **In-process only**: two replicas over one database still race, which needs an advisory lock in the store or an ACL sharing the metadata transaction.
...
- **Python parity notes**: 418 envelope is intentional. Granting all four ACLs synchronously is critical — if any of the four `give_permission_on_dataset` calls fails, Python catches the exception and returns 418, leaving partial ACLs. **Rust deliberately diverges and does not leave them.** A single transaction is not available — `AclDb` is a separate trait over a possibly separate store — so the handler compensates instead: revoke all four (idempotent), then drop the row it just wrote, addressed **by id**. Three deliberate refinements of that:
  - The row is dropped **only if the revokes all succeeded**. When the ACL store is unreachable the revokes fail for the same reason the grant did, and deleting anyway would leave surviving grants on a deterministic id that the *next* create of the same name would silently inherit. In that case the row is kept and the error names it for manual cleanup.
  - The already-exists arm never re-grants. That would turn an idempotent create into an ACL reset, letting an owner restore a deliberately revoked `read` by re-POSTing the name.
  - The rollback does **not** go through `delete_service`, which SDK-636 originally proposed for its `dataset_data` sweeping. `DeleteScope::Dataset` resolves by *name* and passes `tenant_id: None`, which applies no tenant predicate — so in a tenanted deployment it could hard-delete a same-named dataset belonging to another tenant; and `DeleteMode::Hard` runs a graph-*wide* degree-one `Entity`/`EntityType` purge that is not scoped to the dataset at all. Under the identity lock the row is empty anyway, so a row delete orphans nothing; the handler still checks for attached data first and **keeps** the row if any is found, since the lock does not span replicas.

  ⚠️ **The invariant holds for this handler, not for every create path.** `persist_data_with_acl` (the `add`/`remember`/`update`/`improve` route into dataset creation) still propagates a failed grant with `?` and leaves its row behind uncompensated. A later `POST /api/v1/datasets` for that name then takes the already-exists arm, does not re-grant, and returns 200 indefinitely for a dataset no ACL-aware path can read. Pre-existing and untouched here — worth its own ticket.
