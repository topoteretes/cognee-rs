...
- **Permission gate**: `read` on each candidate dataset (the SDK call already filters; the handler does not re-check). [../tenants.md §5](../tenants.md#5-permission-resolution).
  ...
  **Which source answers is decided by whether an `AclDb` is wired, never by what the ACL returned (SDK-637).**

  - **`AclDb` wired** (the cloud build): `authorized_dataset_ids_with_roles(user.id, "read")` is the whole answer, the empty list included. This matches Python — `get_datasets_router.py` returns whatever `get_all_user_permission_datasets(user, "read")` gives — and matches `SearchOrchestrator::readable_dataset_ids`, which has consulted the ACL and nothing else since [PR 214](https://github.com/topoteretes/cognee-rs/pull/214).
  - **No `AclDb`** (OSS single-user, Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` default): `IngestDb::list_datasets_by_owner(user.id)`, filtered on the caller's tenant. `list_datasets_by_owner` spans every tenant the owner appears in and the bindings let one handle write under several, so the predicate mirrors Python's `dataset.tenant_id == user.tenant_id` and the identical filter in search's fallback. A `None` tenant applies no filter — the single-tenant default every OSS row is written under, so the OSS path is unchanged.

  ⚠️ **This was a behaviour change for deployments relying on the old fallback.** The guard used to be `if datasets.is_empty()`, so the ownership listing fired *through* a live ACL: a caller whose `read` grants were revoked or never written got the full ownership listing here and a `403` from `POST /v1/search` for the same datasets. Removing it means a cloud tenant whose ACL rows were never backfilled now sees an empty list rather than reading through to ownership — correct, and what search has always answered, but visible. The handler names that case rather than leaving it silent: when the ACL returns nothing **and** the caller owns rows, it logs a `warn!` carrying `user_id` and `owned_datasets`, which is the signal that the ACL needs a backfill. A caller who genuinely owns no datasets logs nothing.
