...
- **Error responses**:
  ...
  | 403 | `{"detail": "Request owner does not have necessary permission: [read] for all datasets requested. [PermissionDeniedError]"}` | A `dataset_ids` entry the caller may not read (someone else's, or nonexistent). Python re-raises `PermissionDeniedError` into the global `CogneeApiError` handler ([`get_search_router.py:290-295`](https://github.com/topoteretes/cognee/blob/main/cognee/api/v1/search/routers/get_search_router.py#L290-L295), [`client.py:220-236`](https://github.com/topoteretes/cognee/blob/main/cognee/api/client.py#L220-L236)), so the body is the canonical `{detail}` shape rather than `ErrorResponseDTO`. [`recall`](recall.md) answers identically. |
...
- **Permission gate**: `SearchOrchestrator::authorize_dataset_ids` checks every dataset id in scope against the caller's readable set; one miss — unreadable or nonexistent — fails the whole batch with `SearchError::PermissionDenied` → `403` (Python: `get_specific_user_permission_datasets` raises when `len(permitted) != len(requested)`). Unknown and foreign ids are deliberately indistinguishable, so the caller gets no existence oracle. It runs on **both** paths — the caller's explicit `dataset_ids`, and the ids that `datasets` names resolve to — matching Python, whose `get_authorized_existing_datasets` pipes `get_dataset_ids(datasets, user)` straight into the same permission filter. What counts as "readable":
  - **When an `AclDb` is wired** (the closed/cloud build), readability comes from `authorized_dataset_ids_with_roles(user.id, "read")` — direct, tenant- and role-inherited grants are all honoured, matching Python's `get_all_user_permission_datasets`. There is no *ownership* fallback: owning a dataset grants nothing by itself, so an owner with no `read` reaching them by any route — direct, tenant or role — is denied, by id and by name alike, exactly as in Python. (An owner who lost their direct row but sits in a tenant or role that holds `read` is still admitted; that is the ACL working, not a fallback.) That is why `POST /v1/datasets` fails the create — and rolls the row back — when the owner grant cannot be written.
  - **When no `AclDb` is wired** (OSS single-user), readability falls back to `list_datasets_by_owner(user.id)`, filtered on the caller's tenant — "readable" is "owned by the caller *and* in their tenant", matching Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` default. `list_datasets_by_owner` spans every tenant the owner appears in, so without that filter a caller scoped to tenant A could pass tenant B's id and pass the check. `GET /v1/datasets` runs the identical predicate on its own no-ACL path (SDK-637), so the two agree. Names are resolved owner-scoped in the first place, so on this path the check never rejects anything it did not already resolve. Note the predicate is `is_none_or`, **not** Python's `dataset.tenant_id == user.tenant_id`: the two coincide for a tenanted caller, but an untenanted one admits tenanted rows here and is refused them by Python — an OSS-compat choice, since `None` means "named no tenant" and `==` would hide every tenanted row from the default user.
  - ⚠️ **Neither path applies Python's tenant predicate on the ACL side.** `get_all_user_permission_datasets.py` filters the deduplicated union of direct, tenant and role grants by `dataset.tenant_id == user.tenant_id`; `readable_dataset_ids` returns the grants as-is. A caller in tenant A holding a direct grant on a tenant-B row is therefore readable here and not in Python. `GET /v1/datasets` has the identical gap on purpose, so the two SDK surfaces agree with each other; closing it is a change to this gate and belongs with that handler, not in one of them alone.
  - **When neither is wired** (an in-process embedder that wired no metadata DB at all), the gate is **skipped** and explicit ids pass through unchecked — the gate guards on `acl_db.is_some() || dataset_resolver.is_some()`. This is a deliberate escape hatch for embedders, not a fail-closed default: there is no store to authorize against. `readable_dataset_ids` itself returns an empty set in that state, so nothing silently widens if the guard is ever removed. (`search_orchestrator.rs:394-396`.)
- **Dataset resolution**: `datasets` names resolve owner-scoped **and** tenant-scoped — `get_dataset_by_name(name, user.id, user.tenant_id)`; none resolving → `DatasetNotFound` → 422, a partial miss is logged and the search proceeds with the resolved subset (which is then authorized as above). Owner-scoping by name is deliberate parity, not a gap: Python's `get_dataset_ids` states "If a user wants to write to a dataset he is not the owner of it must be provided through UUID", so **a dataset shared with the caller is reachable by id and not by name in both SDKs** — do not "fix" this.
