...
- **Permission gate**: enforced in `SearchOrchestrator::search` against the wired `dataset_resolver`. `datasets` names resolve owner-scoped (`get_dataset_by_name(name, user.id)`; none resolving → `DatasetNotFound` → 422). Explicit `dataset_ids` are each looked up and must be owned by `user.id`; one miss — foreign or nonexistent — fails the whole batch with `SearchError::PermissionDenied` → `403` (Python: `get_specific_user_permission_datasets` raises when `len(permitted) != len(requested)`). OSS wires no ACL grants, so "readable" is "owned by the caller"; the closed build's ACL sharing is not yet consulted here.
