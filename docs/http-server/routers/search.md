...
- **Permission gate**: `SearchOrchestrator::authorize_dataset_ids` checks every dataset id in scope against the caller's readable set; one miss — unreadable or nonexistent — fails the whole batch with `SearchError::PermissionDenied` → `403` (Python: `get_specific_user_permission_datasets` raises when `len(permitted) != len(requested)`). Unknown and foreign ids are deliberately indistinguishable, so the caller gets no existence oracle. It runs on **both** paths — the caller's explicit `dataset_ids`, and the ids that `datasets` names resolve to — matching Python, whose `get_authorized_existing_datasets` pipes `get_dataset_ids(datasets, user)` straight into the same permission filter. What counts as "readable":
  ...
  - **When no `AclDb` is wired** (OSS single-user), readability falls back to `list_datasets_by_owner(user.id)` — "readable" is "owned by the caller", matching Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` default. Names are resolved owner-scoped in the first place, so on this path the check never rejects anything it did not already resolve.
