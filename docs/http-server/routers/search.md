...
- **Permission gate**: enforced in `SearchOrchestrator::search`. What counts as "readable" depends on what is wired:
  - **When an `AclDb` is wired** (the closed/cloud build), readability comes from `authorized_dataset_ids_with_roles(user.id, "read")` — direct, tenant- and role-inherited grants are all honoured, matching Python's `get_all_user_permission_datasets`. There is no ownership fallback: an owner whose `read` row is missing is denied, exactly as in Python. That is why `POST /v1/datasets` fails the create when the owner grant cannot be written rather than returning 200 with an unreadable dataset.
