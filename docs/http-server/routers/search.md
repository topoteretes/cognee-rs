...
- **Permission gate**: enforced in `SearchOrchestrator::search`, and it applies to **explicit `dataset_ids` only** (`search_orchestrator.rs:341-374`). Each supplied id is checked against the caller's readable set; one miss — unreadable or nonexistent — fails the whole batch with `SearchError::PermissionDenied` → `403` (Python: `get_specific_user_permission_datasets` raises when `len(permitted) != len(requested)`). Unknown and foreign ids are deliberately indistinguishable, so the caller gets no existence oracle. What counts as "readable":
  - **When an `AclDb` is wired** (the closed/cloud build), readability comes from `authorized_dataset_ids_with_roles(user.id, "read")` — direct, tenant- and role-inherited grants are all honoured, matching Python's `get_all_user_permission_datasets`. There is no ownership fallback on this path: an owner whose `read` row is missing is denied by id, exactly as in Python. That is why `POST /v1/datasets` fails the create when the owner grant cannot be written rather than returning 200 with a dataset no ACL-aware reader can see.
  - **When no `AclDb` is wired** (OSS single-user), readability falls back to `list_datasets_by_owner(user.id)` — "readable" is "owned by the caller", matching Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` default. With neither an ACL nor a resolver wired the set is empty, i.e. it fails closed.
- **Dataset resolution**: `datasets` names resolve owner-scoped **and** tenant-scoped — `get_dataset_by_name(name, user.id, user.tenant_id)`; none resolving → `DatasetNotFound` → 422, a partial miss is logged and the search proceeds with the resolved subset. Owner-scoping by name is deliberate parity, not a gap: Python's `get_dataset_ids` states "If a user wants to write to a dataset he is not the owner of it must be provided through UUID", so **a dataset shared with the caller is reachable by id and not by name in both SDKs** — do not "fix" this.
  ⚠️ **Names resolve on ownership, and the resolved ids are not re-checked against the readable set.** The name branch runs *after* the id gate above and writes straight into `dataset_ids`, so with an `AclDb` wired an owner whose `read` grant is missing is refused by id yet still served by name. The window is narrow — it needs ownership without a grant, which is the half-finished-create state `POST /v1/datasets` now refuses to produce — but it is a real divergence from Python's `get_authorized_existing_datasets`, which filters resolved names by permission too. Tracked separately; do not read the gate above as covering the name path.
