...
- **Permission gate**: `read` permission on each requested dataset (same as search — see [search.md §Permission gate](search.md) for the full rules). Names resolve owner- and tenant-scoped; explicit `dataset_ids` are checked against the caller's readable set in `SearchOrchestrator::search` — ACL grants (direct, tenant and role) when an `AclDb` is wired, ownership when one is not (OSS, matching Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` default). Any denied id fails the whole batch with `SearchError::PermissionDenied`, which the handler maps to **403** exactly as search does. `dataset_ids` takes precedence over `datasets` when both are sent; an empty list is no filter. The binding-level `recall` op forwards `datasetIds` the same way the `search` op does.
