...
- **Permission gate**: `read` permission on each **explicitly supplied `dataset_ids` entry** — same rules as search, see [search.md §Permission gate](search.md), including the caveat that name-resolved ids are *not* re-checked. Ids are checked against the caller's readable set in `SearchOrchestrator::search`: ACL grants (direct, tenant and role) when an `AclDb` is wired, ownership when one is not (OSS, matching Python's `ENABLE_BACKEND_ACCESS_CONTROL=false` default). Any denied id fails the whole batch with `SearchError::PermissionDenied`, which the handler maps to **403** exactly as search does. Names resolve owner- and tenant-scoped. `dataset_ids` takes precedence over `datasets` when both are sent; an empty list is no filter. The binding-level `recall` op parses `datasetIds` out of its raw opts the same way the `search` op does — note that the *typed* wrappers expose the field on search only, so a TS or Java caller reaches it through the untyped opts.
