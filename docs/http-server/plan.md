...
| 8 | [e2e-parity.md](e2e-parity.md) | Cross-SDK HTTP parity harness — Python uvicorn ↔ `cognee-http-server` side-by-side in one Docker container, pytest + `httpx` clients, structural JSON diff with field-strip allowlist. 27 test files mapped to phases. | **Done** (implemented in P8, commit 2faf3ac) |
...
|---|---|---|---|---|
| **P0** ✓ | [p0-foundation.md](implementation/p0-foundation.md) | Crate skeleton, `AppState`, `ApiError`, CORS, OpenAPI bootstrap, root `/`, `cognee-http-server` standalone binary, integration test that hits `/health`. **Done** (commit 323e3e1). | 1 day | [health](routers/health.md) |
| **P1** ✓ | [p1-auth.md](implementation/p1-auth.md) | JWT auth + cookie + `X-Api-Key`, login / logout / me, AuthUser extractor, API-key extractor. Migrations for `users` and `user_api_key`. Stubbed register / reset / verify returning `501`. **Done** (commit 0459963). | 2 days | [auth](routers/auth.md), [auth-register](routers/auth-register.md), [auth-reset-password](routers/auth-reset-password.md), [auth-verify](routers/auth-verify.md), [api-keys](routers/api-keys.md), [users](routers/users.md), [users-by-email](routers/users-by-email.md) |
| **P2** ✓ | [p2-write-path.md](implementation/p2-write-path.md) | Write path, multipart streaming. **Done** (commit 3b4ae9e). | 3 days | [add](routers/add.md), [update](routers/update.md), [datasets](routers/datasets.md), [ontologies](routers/ontologies.md), [delete](routers/delete.md), [forget](routers/forget.md) |
| **P3-prereq** ✓ | [p3-prereq-library-refactor.md](implementation/p3-prereq-library-refactor.md) | Library refactor: drop `run_in_background` from `cognee_lib::api::remember()` and `improve()`. Land `cognee_core::PipelineRunRegistry` + `PipelineRunRepository` trait. **Done** (commit 2425f19). | 2 days | — (library work; touches `cognee-core`, `cognee-database`, `cognee-lib`) |
| **P3** ✓ | [p3-pipelines-and-websocket.md](implementation/p3-pipelines-and-websocket.md) | Pipelines + WebSocket. Requires the **library refactor prerequisite** to land first. **Done** (commit 53b1da0). | 3 days | [cognify](routers/cognify.md), [memify](routers/memify.md), [remember](routers/remember.md), [improve](routers/improve.md) |
| **P4** ✓ | [p4-read-path.md](implementation/p4-read-path.md) | Read path. **Done** (commit 3e10c70). | 2 days | [search](routers/search.md), [recall](routers/recall.md), [llm](routers/llm.md), [visualize](routers/visualize.md) |
| **P5** ✓ | [p5-admin.md](implementation/p5-admin.md) | Admin + RBAC. SeaORM migration for `tenants`, `roles`, `user_roles`, `user_tenants`, `permissions`, `acls`, default-permission tables. **Done** (commits aefb105 + 2652aea). | 3 days | [permissions](routers/permissions.md), [settings](routers/settings.md), [configuration](routers/configuration.md) |
| **P6** ✓ | [p6-observability.md](implementation/p6-observability.md) | Observability. In-memory span buffer for `/activity/spans`. **Done** (commit 0e928bf). | 2 days | [activity](routers/activity.md), [sync](routers/sync.md), [checks](routers/checks.md) |
| **P7** ✓ | [p7-advanced.md](implementation/p7-advanced.md) | Advanced. `/notebooks` storage only (sandbox `/run` returns 501). `/responses` stubs as 501. SMTP `Mailer` impl. **Done**. | 2 days | [notebooks](routers/notebooks.md), [responses](routers/responses.md) |
| **P8** ✓ | [p8-e2e-parity.md](implementation/p8-e2e-parity.md) | Cross-SDK HTTP parity harness. All three waves (phase-1 + phase-2 + phase-3) shipped in one commit. **Done** (commit 2faf3ac). | 2 days | — (see [e2e-parity.md](e2e-parity.md)) |
