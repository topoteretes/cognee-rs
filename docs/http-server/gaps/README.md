...
| **7** | `POST /api/v1/remember/entry` (`generate_feedback_with_llm` path) | [routers/remember.rs:724-740](../../../crates/http-server/src/routers/remember.rs#L724-L740) | **not-started** ([plan](impl/11-remember-feedback-llm.md)) | When `generate_feedback_with_llm: true`, the handler logs the request and proceeds with an **empty string** as `session_feedback` rather than generating it via an LLM. Live marker: `// TODO(LIB-01-followup): generate_feedback_with_llm requires wiring an Arc<dyn Llm> + prompt template through SessionManager.` (L724). Plan generates feedback in the handler (not in `SessionManager`) using the existing `ComponentHandles.llm` slot, with timeout, graceful degradation, and response scrubbing. |
