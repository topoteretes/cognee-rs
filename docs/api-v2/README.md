...
| `forget()` | Unified deletion: by item id, by dataset, or everything. Cascades across relational DB, graph DB, vector DB, file storage, and session cache. Supports ACL enforcement and dry-run preview. | **Implemented** — `crates/lib/src/api/forget.rs` is a thin facade over `DeleteService` in `crates/delete/`, which covers all three modes including session `prune()` for the `everything` scope. | [forget.md](forget.md) | [impl/forget-plan.md](impl/forget-plan.md) (polish only) |
