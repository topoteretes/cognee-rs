...
| `edge-reindex` | Report, and with `--apply` write, the `EdgeType_relationship_name` vector rows the graph implies but the vector store lacks — the orphans a run killed between cognify's graph write and its vector write leaves behind, which a retry never re-emits (the edge is already in the graph, so it is routed into the claim-only bucket) and the rollback sweeper never reaches (it runs on in-process failures only). Reports by default because the count is free (an id probe embeds nothing) while applying bills one embedding per missing row. The orphan count is per *distinct `EdgeType` point*, not per edge — many edges share one row, and retrieval texts differing only in case, spacing or apostrophes normalise to the same point and are counted once with their edge counts summed. **It is not a crash-damage count**: DLT foreign-key edges, and structural edges (`is_part_of`, `contains`, `made_from`, `is_a`) cognified before cognify stamped them with an `edge_text` sentence, have no row on a healthy graph either — expect a small constant floor, with real crash orphans (sentence-shaped texts) on top. Applying writes the floor too, making those edges rankable instead of always taking the distance penalty, which is what Python's whole-graph `index_graph_edges` does as well. The scan always loads the whole graph into memory; `--limit`/`--resume-after` bound the embedding work, not the read. All backends | `--apply`, `--dataset-id` (**label only — does not scope the scan, and cannot: the point id is hashed from the retrieval text alone, so one row is shared across datasets**), `--limit` (writes only; no effect without `--apply`; must be `>= 1` — a zero budget would report no resume cursor), `--resume-after` (cursor from a previous run; narrows the reported counts too) | — |
