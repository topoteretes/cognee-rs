...
**12 tasks complete (CLEAN-01, LIB-06, E-01, E-03, E-06, E-07, E-08, LIB-02, LIB-03, LIB-05, LIB-04, LIB-01). Resume point: TASK B-6 (LIB-07) — Phase B extended by Decision 17 to add a recall-scope library widening before E-04.**

The v2 doc package landed in commits ending `…/docs/http-api-v2/`. The port now has **20 tasks** total (Decision 17 added LIB-07). 8 of 20 tasks are at status **Not Started** (1 cleanup + 6 library prerequisites + 5 endpoints done; 1 new library prerequisite + 7 endpoints remain; CLEAN-01 e146835, LIB-06 b39cd05, E-01 037cad2, E-03 0dafdee, E-06 verified-no-code-change, E-07 35d6b3c, E-08 afa048f, LIB-02 eec6f79, LIB-03 82728f2, LIB-05 60c934a, LIB-04 9f1879e, LIB-01 0818644).
...
|   | B-6 | [LIB-07](tasks/lib-07-recall-scope-widening.md) | **Not Started.** Widen `cognee_lib::api::recall::recall()` to accept `scope: Option<Vec<RecallScope>>` and implement source fan-out across `graph` / `session` / `trace` / `graph_context`. New `RecallScope` enum + `normalize_scope()` helper + `_search_session` / `_search_trace` / `_fetch_graph_context` private helpers. Per **Decision 17** (2026-04-30), this was split out of E-04 so that E-04 retains strict Python parity (no D-2 wire divergence). Blocks E-04. |
| **C — Partial endpoints** | C-1 | [E-04](tasks/e-04-recall-search.md) | `POST /recall` — add `session_id` + `scope` (depends on **B-6 LIB-07** for the library widening; investigation 2026-04-30 surfaced the gap; user chose split per Decision 17). |
