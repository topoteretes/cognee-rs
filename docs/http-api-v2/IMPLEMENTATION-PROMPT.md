...
**13 tasks complete (CLEAN-01, LIB-06, E-01, E-03, E-06, E-07, E-08, LIB-02, LIB-03, LIB-05, LIB-04, LIB-01, LIB-07). Resume point: TASK B-7 (LIB-08) — Phase B extended again by Decision 18 to lift LIB-07's primitives out of `cognee-lib` (cycle constraint).**

The v2 doc package landed in commits ending `…/docs/http-api-v2/`. The port has **21 tasks** total (Decision 17 added LIB-07; Decision 18 added LIB-08). 8 of 21 tasks are at status **Not Started** (1 cleanup + 7 library prerequisites + 5 endpoints done; 1 new library prerequisite + 7 endpoints remain; CLEAN-01 e146835, LIB-06 b39cd05, E-01 037cad2, E-03 0dafdee, E-06 verified-no-code-change, E-07 35d6b3c, E-08 afa048f, LIB-02 eec6f79, LIB-03 82728f2, LIB-05 60c934a, LIB-04 9f1879e, LIB-01 0818644, LIB-07 7d25c0b).
...
|   | B-7 | [LIB-08](tasks/lib-08-recall-scope-lift.md) | **Not Started.** Lift `RecallScope`, `ScopeInput`, `normalize_scope`, `RecallSource` (with `Trace`/`GraphContext`), and the four source helpers from `cognee-lib::api::recall` to `cognee-search` (or another lower-level crate). Re-export from `cognee-lib` so the public API stays stable. Per **Decision 18** (2026-04-30), this resolves the http-server↔lib cycle that E-04's consumer-side discovered: `cognee-http-server` cannot import from `cognee-lib`, so LIB-07's helpers cannot be reached from the HTTP layer at their current location. Pure relocation + re-export — no behavior change. Blocks E-04. |
...
| `cognee_lib::api::recall::{RecallScope, normalize_scope, ScopeInput}` + extended `RecallSource` enum (`Trace`, `GraphContext`) + widened `recall()` signature with `scope: Option<Vec<RecallScope>>` and `session_manager: Option<&SessionManager>` + four-source fan-out (`graph`/`session`/`trace`/`graph_context`) with Python-parity `auto_fallthrough` short-circuit (Python `recall.py:374-386, 508-509`) | LIB-07 (Decision 17; landed 7d25c0b) | E-04 (`POST /recall` adds `session_id` + `scope`) |
