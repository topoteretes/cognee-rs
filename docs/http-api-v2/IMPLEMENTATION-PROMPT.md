...
**18 tasks complete (CLEAN-01, LIB-06, E-01, E-03, E-06, E-07, E-08, LIB-02, LIB-03, LIB-05, LIB-04, LIB-01, LIB-07, LIB-08, E-04, E-05, E-02, E-09). Resume point: TASK D-3 (E-10) — `GET /sessions/stats` (depends on LIB-05, landed; consumes E-09's `/sessions` router-mount + `ValidatedQuery<T>` extractor).**
...
| **C — Partial endpoints** | C-1 | [E-04](tasks/e-04-recall-search.md) | `POST /recall` — add `session_id` + `scope`. **Done (commit 9981e79).** Extended `RecallPayloadDTO` with `sessionId` + `scope`; replicated Python's auto-resolution + iteration + `auto_fallthrough` logic inline in the handler via `cognee_search::recall_scope::{search_session, search_trace, fetch_graph_context, run_graph}` (Decision 18 cycle workaround). Wire shape mirrors Python (flat list of dicts each with `_source` ∈ {session, trace, graph_context, graph}). `ComponentHandles` extended with `session_store` + `session_manager`. 6 DTO unit tests + 5 new integration tests + 3 cross-SDK harness tests. **No new wire divergence**. Reviewer-amended: graph_context content key fixed from `snapshot` → `content` per Python `recall.py:314`. |
|   | C-2 | [E-05](tasks/e-05-improve.md) | `POST /improve` — add `session_ids` + extraction/enrichment/data/node_name (consumes B-4's `ImproveParams`). **Done (commit 43e2a72).** DTO extended from 3 to 8 fields (added `extractionTasks`, `enrichmentTasks`, `data`, `nodeName`, `sessionIds` with camelCase wire + snake_case alias per Decision 10); `ImproveParams` extended with 3 new fields wired through to all 5 call sites (no `Default` derive per LIB-04 option (b)); handler tracing scope adds `session_ids_count`/`extraction_tasks_count`/`enrichment_tasks_count`/`node_name_count`/`has_data` per Python parity at `get_improve_router.py:67-74`. The no-op handler stub stays — wiring the real `improve()` library call is the deferred P5 follow-up (cycle constraint + missing `ComponentHandles` slots). 5 new integration + 4 DTO unit + 4 lib regression tests + cross-SDK harness `test_http_v2_improve.py`. **No new wire divergence.** |
| **D — Missing endpoints** | D-1 | [E-02](tasks/e-02-remember-entry.md) | **Done (commit 75c0886).** New `POST /remember/entry` endpoint. `RememberEntryRequestDTO` consumes `cognee_models::memory::MemoryEntry` directly (no wrapper); inline replication of `cognee_lib::api::remember::remember_entry` (cycle-constraint workaround per §3.1, mirrors E-04's pattern). Added `entry_type`/`entry_id` to `RememberResultDTO` per Decision 5. Added `ApiError::ServiceUnavailable` (503) for missing session cache. 7 integration tests + cross-SDK harness. **No new wire divergence.** |
