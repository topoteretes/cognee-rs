...
| T2  | S2 access-control extraction (trait OSS / impl closed; S2b migration cleave; S2c DB-free user; S2d remove `dyn AclDb` casts; S2e move 13 entity files + Role/Tenant). | §4 S2, S2b–e      | both   | in-progress — **T2-pre done** (S2c DB-free default user keyed by `uuid5(NAMESPACE_OID, email)` for Python parity; S2d 4 prod ACL casts → injected `Option<Arc<dyn AclDb>>` via new `ComponentHandles.acl_db` slot; 9 `UserDb::get_user(owner_id)` prod sites → `None`; `--enforce-acl` CLI flag now errors out; `core_migrations()` accessor on OSS migrator; 5 test sites → `MockAclDb`; full `check_all.sh` green incl. C/Python/JS bindings). **T2-move next** (physical entity/trait/op/model/mock/auth/permissions move to closed). |
