...
//!
//! This is the keystone facade for the TypeScript SDK bindings: every later
//! `sdk_*` function obtains a `CogneeServices` via `CogneeHandle::services()` and
//! calls a `cognee-lib` API with the bundled `Arc<dyn …>` handles, so the wiring
//! lives in exactly one place (mirroring the CLI command builders, which are the
...
// Most fields are consumed by the SDK ops added in Phases 3–6; they are part of
// the facade contract now so the wiring lives in one place.
#[allow(dead_code)]
...
impl CogneeServices {
    ...
    pub async fn build(cm: &ComponentManager) -> Result<(Self, Uuid), SdkError> {
        ...
        // Unauthorized DeleteService; the ACL-enforcing wrapper is a Phase-5
        // concern.
    ...
    #[allow(dead_code)] // used by cognify in Phase 3
