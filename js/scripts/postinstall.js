...
// ships JS only (the `cognee-neon/` Rust source and the wider workspace are not
...
//   • From a git checkout (the `cognee-neon/` crate is present), it falls back
...
/**
 * Returns true when a locally-built `cognee_neon.node` exists next to this
 * package (produced by a previous `npm run build:rust`).
 */
function localBuildExists() {
  const nodeFile = path.join(__dirname, "..", "cognee_neon.node");
...
//    we're done.
if (localBuildExists()) {
  console.log("cognee: found local cognee_neon.node — skipping source build.");
  process.exit(0);
}
...
// missing directory.
const hasRustSource = fs.existsSync(path.join(pkgRoot, "cognee-neon", "Cargo.toml"));
if (!hasRustSource) {
  console.error(
    ...
    "    `cd js && npm run build` (requires a Rust toolchain).\n",
  );
